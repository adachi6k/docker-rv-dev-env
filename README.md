...
| **Build and push Docker image** | Push to `main` that changes `Dockerfile` or `.github/workflows/build-push.yml` / manual | Builds the Docker image, runs runtime smoke tests for Verilator and required development packages, and pushes it to GHCR with the `latest` tag, a date tag (`YYYYMMDD`), and a short-SHA tag. |
